...
# Dev Keys: (Uncomment these lines if you want your 3ds rom to be encrypted with Dev Keys instead)
                    ...
                    if (partition_flags[7] & 0x04): # check if the 'NoCrypto' bit (bit 3) is set
                        ...
                        if magic == "NCCH": # check if partition is valid
                            ...
                            if (exhdr_len[0] > 0):
                                ...
                                print ("Partition %1d ExeFS: Encrypting: ExHeader") % (p)
                                g.write(exefsctrmode2C.encrypt(f.read(exhdr_filelen)))
                            ...
                            if (exefs_len[0] > 0):
                                        ...
                                        if str(exefs_filename[0]) == str(".code\x00\x00\x00"):
                                            ...
                                            if (datalenM > 0):
                                                for i in xrange(datalenM):
                                                    g.write(exefsctrmode2C.decrypt(exefsctrmode.encrypt(f.read(1024*1024))))
                                                    print ("\rPartition %1d ExeFS: Encrypting: %8s... %4d / %4d mb...") % (p, str(exefs_filename[0]), i, datalenM + 1),
                                            if (datalenB > 0):
                                                g.write(exefsctrmode2C.decrypt(exefsctrmode.encrypt(f.read(datalenB))))
                                            print ("\rPartition %1d ExeFS: Encrypting: %8s... %4d / %4d mb... Done!") % (p, str(exefs_filename[0]), datalenM + 1, datalenM + 1)
                                ...
                                exefsctrmode2C = AES.new(to_bytes(NormalKey2C), AES.MODE_CTR, counter = exefsctr2C)
                                g.write(exefsctrmode2C.encrypt(f.read(sectorsize)))
                                print ("Partition %1d ExeFS: Encrypting: ExeFS Filename Table") % (p)
                                ...
                                if (exefsSizeM > 0):
                                    for i in xrange(exefsSizeM):
                                        g.write(exefsctrmode2C.encrypt(f.read(1024*1024)))
                                        print ("\rPartition %1d ExeFS: Encrypting: %4d / %4d mb") % (p, i, exefsSizeM + 1),
                                if (exefsSizeB > 0):
                                    g.write(exefsctrmode2C.encrypt(f.read(exefsSizeB)))
                                print ("\rPartition %1d ExeFS: Encrypting: %4d / %4d mb... Done") % (p, exefsSizeM + 1, exefsSizeM + 1)
                            ...
                            if (romfs_off[0] != 0):
                                ...
                                if (romfsSizeM > 0):
                                    for i in xrange(romfsSizeM):
                                        g.write(romfsctrmode.encrypt(f.read(romfsBlockSize*(1024*1024))))
                                        print ("\rPartition %1d RomFS: Encrypting: %4d / %4d mb") % (p, i*romfsBlockSize, romfsSizeTotalMb),
                                if (romfsSizeB > 0):
                                    g.write(romfsctrmode.encrypt(f.read(romfsSizeB)))
                                    
                                print ("\rPartition %1d RomFS: Encrypting: %4d / %4d mb... Done") % (p, romfsSizeTotalMb, romfsSizeTotalMb)
                            ...
                            if (p > 0):
                                g.write(struct.pack('<B', int(0x00))) # for partitions 1 and up, set crypto-method to 0x00 (Key0x2C)
                            else:
                                g.write(struct.pack('<B', int(backup_flags[3]))) # if partition 0, restore crypto-method from backup_flags[3]
                            
                            g.seek((part_off[0] * sectorsize) + 0x18F)
                            flag = int(partition_flags[7]) # read partition flag
                            flag = (flag & ((0x01|0x20|0x04)^0xFF)) # turn off 0x01 = FixedCryptoKey and 0x20 = CryptoUsingNewKeyY and 0x04 = NoCrypto
                            flag = (flag | (0x01|0x20)&backup_flags[7]) # set bits of crypto-method to backup flags[7] bits
                            g.write(struct.pack('<B', int(flag))) # write flag

                        else:
                            print ("Partition %1d Unable to read NCCH header") % (p)
                    else:
                        print ("Partition %1d: Already Encrypted?...") % (p)
                else:
